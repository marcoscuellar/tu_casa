...
// functions-src/build-resume.ts
var build_resume_exports = {};
__export(build_resume_exports, {
  default: () => handler,
  maxDuration: () => maxDuration
});
module.exports = __toCommonJS(build_resume_exports);
...
// src/engines/live/aiResumeBuilder.ts
var AI_BUILDER_SYSTEM = `You are TuCasa's r\xE9sum\xE9 builder \u2014 a warm, plain-spoken assistant helping someone who has NO r\xE9sum\xE9 create one, just by chatting. Many of these people are early-career, career-switchers, or coming back to work; keep it human and encouraging, never corporate.

Your job each turn:
1. Ask exactly ONE short, plain-language question to gather what a r\xE9sum\xE9 needs \u2014 the kind of work they want, their past jobs (what they did day to day), how long, the industry, and their skills. One question at a time, like a friendly person, not a form. (e.g. "What kind of work are you looking for?", "What did a normal day look like?", "What's your name so we can put it on top?")
2. From EVERYTHING they've said so far, maintain a structured r\xE9sum\xE9 draft. Turn their plain answers into polished, concrete r\xE9sum\xE9 lines (strong verbs, numbers when they gave them) \u2014 but NEVER invent facts, employers, or metrics they didn't state.
3. When you have enough for a solid one-pager, make your reply a short wrap-up (e.g. "That's a real r\xE9sum\xE9 now \u2014 tap Use this r\xE9sum\xE9 when you're ready, or keep editing.").
...
async function handler(req, res) {
  try {
    ...
    const clean = messages.filter((m) => (m.role === "user" || m.role === "assistant") && typeof m.content === "string").map((m) => ({ role: m.role, content: m.content.slice(0, 4e3) }));
    ...
    const msg = err instanceof Error ? err.message : "Unexpected error building the r\xE9sum\xE9.";
