...
// functions-src/parse-resume.ts
var parse_resume_exports = {};
__export(parse_resume_exports, {
  default: () => handler,
  maxDuration: () => maxDuration
});
module.exports = __toCommonJS(parse_resume_exports);
...
var ResumeParseError = class extends Error {
};
...
function validateParsedResume(raw, opts = {}) {
  const asOfYear = opts.asOfYear ?? (/* @__PURE__ */ new Date()).getFullYear();
  if (!raw || typeof raw !== "object") {
    throw new ResumeParseError("Could not read the r\xE9sum\xE9 \u2014 no data returned.");
  }
  const r = raw;
  const rawTitles = Array.isArray(r.titles) ? r.titles : [];
  const titles = rawTitles.map((t) => {
    const text = t && typeof t === "object" && typeof t.raw === "string" ? t.raw.trim() : typeof t === "string" ? t.trim() : "";
    if (!text) return null;
  ...
  for (const s of rawSkills) {
    ...
    const years = num(s.years, 0, 60, 0);
    const lastUsed = Math.round(
      num(s.last_used_year, 1970, asOfYear, asOfYear)
    );
...
async function parseResumeWith(input, callModel, opts = {}) {
...
async function handler(req, res) {
  try {
    ...
    if (!apiKey) {
      res.status(500).json({ error: "Server is missing the Anthropic API key (set ANTHROPIC_API_KEY)." });
      return;
    }
    ...
    if (!upload || !hasText && !hasPdf) {
      res.status(400).json({ error: 'Provide a r\xE9sum\xE9 as { kind: "text", text } or { kind: "pdf", base64 }.' });
      return;
    }
...
${input.text}` }];
    ...
    try {
      ...
      if (err instanceof Anthropic.APIError) {
        res.status(502).json({ error: `R\xE9sum\xE9 service error (${err.status}): ${err.message}` });
        return;
      }
      throw err;
    }
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unexpected error parsing the r\xE9sum\xE9.";
